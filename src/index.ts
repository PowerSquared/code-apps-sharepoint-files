...
import type {
  ApiDefinition,
  CreateSharePointFileServiceOptions,
  SharePointFile,
...
type ApiRegistry = Record<string, { apis: Record<string, ApiDefinition> }>;
...
function resolveFileName(file: Blob, options: UploadOptions): string {
  const name = options.fileName ?? (file as File).name;
  if (!name) {
 ...
 * ```
 */
export function createSharePointFileService(
  options: CreateSharePointFileServiceOptions
): SharePointFileService {
  const { dataSourcesInfo, dataSourceName } = options;

  const registry = dataSourcesInfo as unknown as ApiRegistry;
  const dataSource = registry[dataSourceName];
  if (!dataSource) {
    throw new Error(
      `Data source '${dataSourceName}' is not in dataSourcesInfo. ` +
  ...
  const client = getClient(dataSourcesInfo as unknown as Parameters<typeof getClient>[0]);

  return {
    async uploadFile(file, uploadOptions) {
      const result = await client.executeAsync<Record<string, string>, SharePointFile>({
        connectorOperation: {
    ...
    async deleteFile(fileId) {
      const result = await client.executeAsync<Record<string, string>, void>({
        connectorOperation: {
          tableName: dataSourceName,
    ...
    async getFileContent(fileId) {
      const result = await client.executeAsync<Record<string, string | boolean>, Uint8Array>({
        connectorOperation: {
          tableName: dataSourceName,
